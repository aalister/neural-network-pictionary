...
namespace PictionaryAI
{
    public class RoomManager
    {
        ...
        {
            Room room = new Room(totalRounds, countdownMillis, roundLengthMillis, roundBreakMillis);
        ...
        public async Task StartNewRound(IHubContext<PictionaryHub> context, string roomId)
        {
            ...
            await context.Clients.Group(room.Id).SendAsync("NewRound", room.CurrentRound, room.TotalRounds, prompt, promptIndex, roundLengthMillis);
        }
        ...
        public async Task EndRound(IHubContext<PictionaryHub> context, string roomId)
        {
            ...
            {
                //Send the show results
                await context.Clients.Group(room.Id).SendAsync("ShowResults", room.GetUsers().Select(user => new Models.Player(user.Id, user.IsHost, user.Name, user.Score, user.HasCompletedDrawing)));
            }
        }
